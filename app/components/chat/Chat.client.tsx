...
export const ChatImpl = memo(
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const {
      ...
      setData,
      addToolResult,
      id: chatId, // Extract chat ID for Mem0
    } = useChat({
      api: '/api/chat',
      body: {
        apiKeys,
    ...
    // Memory: Load relevant memories when chat starts or context changes
    useEffect(() => {
      const loadMemories = async () => {
        if (!chatId || !chatStarted) {
          console.log('[Memory] Skipping load - chatId:', chatId, 'chatStarted:', chatStarted);
          return;
        }

        const config = memoryConfigHelpers.getConfig();
        console.log('[Memory] Load memories - config:', {
          enabled: config.enabled,
          hasApiKey: !!config.apiKey,
        });

        if (!config.enabled) {
          console.log('[Memory] Memory disabled');
          return;
        }
        ...
        try {
          ...
          if (result.success && result.context) {
            // Update loaded memories in store
            memoryHelpers.clearMemories();
            result.context.memories.forEach((memory) => memoryHelpers.addMemory(memory));
            lastSync.set(Date.now());

            console.log('[Memory] Loaded', result.context.memories.length, 'memories');
            console.log('[Memory] Summary:', result.context.summary);
          }
        } catch (error) {
      ...
      loadMemories();
    }, [chatId, chatStarted, messages.length, model]); // Add model to dependencies to reload on model switch

    // Memory: Auto-save memories when messages change
    useEffect(() => {
      const autoSaveMemories = async () => {
        if (!chatId || !chatStarted || messages.length === 0) {
          return;
        }
        ...
        // Also extract from files if available
        if (files && Object.keys(files).length > 0) {
          const fileMemories = extractor.extractFromFiles(files, chatId);
    ...
    }, [messages, files, chatId, chatStarted]);

    // Memory: Keyboard shortcut for manual save (Ctrl/Cmd + Shift + S)
    useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
        if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'S') {
          ...
          if (!config.enabled || !chatId || !input) {
            return;
          }
          ...
          fetch('/api/memory', {
            ...
            body: JSON.stringify({
              ...
              type: 'highlight',
              importance: 'high',
              metadata: { manualSave: true, timestamp: Date.now() },
            }),
          })
            .then((res) => res.json())
            .then((result) => {
